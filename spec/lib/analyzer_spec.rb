...
RSpec.describe LogAnalyzer::Analyzer do
  ...
  [TEST_FILE, TEST_FILE_7].each do |file|
    context "old #{file} file" do
      before do
      ...
      let!(:analyzer) { LogAnalyzer::Analyzer.new(filename: file) }

      it "runs" do
        expect { analyzer.run }.not_to raise_error
        expect(analyzer.stats.keys).not_to be_empty
      end
